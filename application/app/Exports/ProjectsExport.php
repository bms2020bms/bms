            ...
            foreach (request('standard_field') as $key => $value) {
                if ($value == 'on') {
                    ...
                    case 'project_sum_invoices_all':
                        $map[] = runtimeExcelNumberFormat($projects->sum_invoices_all);
                        break;
                    case 'project_sum_invoices_due':
                        $map[] = runtimeExcelNumberFormat($projects->sum_invoices_due);
                        break;
                    case 'project_sum_invoices_overdue':
                        $map[] = runtimeExcelNumberFormat($projects->sum_invoices_overdue);
                        break;
                    case 'project_sum_invoices_paid':
                        $map[] = runtimeExcelNumberFormat($projects->sum_invoices_paid);
                        break;
                    case 'project_sum_payments':
                        $map[] = runtimeExcelNumberFormat($projects->sum_all_payments);
                        break;
                    case 'project_sum_expenses':
                        $map[] = runtimeExcelNumberFormat($projects->sum_expenses);
                        break;
